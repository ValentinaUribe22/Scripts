...
def open_data(data_type, scenario, start_year, end_year, skip_year):
    ...
    for year in range(start_year, end_year+1):
        ...
        if year != skip_year:
            ...
            data = imod.idf.open(
                f"p:/11207941-005-terschelling-model/TERSCHELLING_IMOD_MODEL_50X50/RESULTS/{scenario}/{data_type}/{file_path}"
...
modelnames = ["maatregel2", "maatregel4", "maatregel3"]# "reference", "reference_riv",
aoi_shape = "data/1-external/aoi/aoi.shp"
path_template = f"data/2-interim/rch_50/template.nc"
...
deelgebieden = {
    "Heel Terschelling" : "data/1-external/aoi/aoi.shp",
    #"Recreatiegebieden": "data/1-external/aoi/Recreatie_Duinrand.shp",
}

for deelgebied, path_shp in deelgebieden.items():
    print(deelgebied)
    ...
    for modelname in modelnames:
        ...
        pathlib.Path(f"data/5-visualization/{modelname}/seepage_-0.5_{deelgebied}").mkdir(
        ...
        for years in year_sets:
            ...
            for name, data in to_stat.items():
                ...
                # Save to CSV
                df.to_csv(f'data/5-visualization/{modelname}/seepage_-0.5_{deelgebied}/{name}_stats_{modelname}_{deelgebied}_{start_year}_{end_year}.csv')
            ...
            for name, data in to_plot.items():
                ...
                colors, levels = imod.visualize.read_imod_legend(
                    "src/5-visualize/legends/kwel_mmd.leg"
                )
                ...
                fig.savefig(
                    f"data/5-visualization/{modelname}/seepage_-0.5_{deelgebied}/{deelgebied}_upward_downward_seepage_{name}.png",
                    dpi=300,
                )
                fig.clf()
                imod.idf.write(
                    f"data/5-visualization/{modelname}/seepage_-0.5_{deelgebied}/{deelgebied}_upward_downward_seepage_{name}.idf",
                    data,
                )
                imod.idf.write(
                    f"data/5-visualization/{modelname}/seepage_-0.5_{deelgebied}/{deelgebied}_upward_downward_seepage_{name}_aoi.idf",
                    data.where(area_raster.notnull()),
                )
                imod.idf.write(
                    f"data/5-visualization/{modelname}/seepage_-0.5_{deelgebied}/{deelgebied}_upward_seepage_{name}.idf",
                    data.where(data > 0),
                )
                imod.idf.write(
                    f"data/5-visualization/{modelname}/seepage_-0.5_{deelgebied}/{deelgebied}_upward_seepage_{name}_aoi.idf",
                    data.where(area_raster.notnull()).where(data > 0),
                )
                imod.idf.write(
                    f"data/5-visualization/{modelname}/seepage_-0.5_{deelgebied}/{deelgebied}_downward_seepage_{name}.idf",
                    data.where(data < 0),
                )
                imod.idf.write(
                    f"data/5-visualization/{modelname}/seepage_-0.5_{deelgebied}/{deelgebied}_downward_seepage_{name}_aoi.idf",
                    data.where(area_raster.notnull()).where(data < 0),
                )
                # Plot
                ...
                # Save
                fig.savefig(
                    f"data/5-visualization/{modelname}/seepage_-0.5_{deelgebied}/{deelgebied}_seepage_{name}.png", dpi=300
                )
                fig.clf()

            ## Zoutvracht
            # create folder to save to
            pathlib.Path(f"data/5-visualization/{modelname}/zoutvracht_-0.5").mkdir(
                exist_ok=True, parents=True
            )

            # Calculate zoutvracht and convert to kg/d/m2
            zoutvracht = (bdgflf * conc / abs(bdgflf["dx"] * bdgflf["dy"])).sel(layer=16)

            # upper_active_layer = imod.select.upper_active_layer(zoutvracht.isel(time=0), is_ibound=False)
            # zoutvracht = zoutvracht.where(zoutvracht.layer == upper_active_layer)
            # zoutvracht = zoutvracht.max("layer")

            mean_total = zoutvracht.mean("time")

            # Select summer (april-september) and winter (oktober-maart) and average last ten years
            jan = zoutvracht.where(zoutvracht.time.dt.month == 1, np.nan)
            feb = zoutvracht.where(zoutvracht.time.dt.month == 2, np.nan)
            mar = zoutvracht.where(zoutvracht.time.dt.month == 3, np.nan)
            apr = zoutvracht.where(zoutvracht.time.dt.month == 4, np.nan)
            may = zoutvracht.where(zoutvracht.time.dt.month == 5, np.nan)
            jun = zoutvracht.where(zoutvracht.time.dt.month == 6, np.nan)
            jul = zoutvracht.where(zoutvracht.time.dt.month == 7, np.nan)
            aug = zoutvracht.where(zoutvracht.time.dt.month == 8, np.nan)
            sep = zoutvracht.where(zoutvracht.time.dt.month == 9, np.nan)
            oct = zoutvracht.where(zoutvracht.time.dt.month == 10, np.nan)
            nov = zoutvracht.where(zoutvracht.time.dt.month == 11, np.nan)
            dec = zoutvracht.where(zoutvracht.time.dt.month == 12, np.nan)
            ...
            to_plot = {
                f"Gemiddelde zoutvracht {start_year}-{end_year}": mean_total,
                f"Gemiddelde zoutvracht zomer {start_year}-{end_year}": summer,
                f"Gemiddelde zoutvracht winter {start_year}-{end_year}": winter,
            }

            # Open data, add CRS, write to .tif
            # imod.idf.open(path)
            # mean_total = mean_total.rio.write_crs("EPSG:28992")
            ...
            for name, data in to_plot.items():
                ...
                plt.axis("scaled")
                fig, ax = imod.visualize.plot_map(
                    zoutvracht.isel(time=100).where(area_raster.notnull()),
                    levels=levels,
                    colors="jet",
                    overlays=overlays,
                    figsize=[12, 6],
                ...
                # Save
                fig.savefig(
                    f"data/5-visualization/{modelname}/zoutvracht_-0.5/{deelgebied}_{name}.png", dpi=300
                )
                fig.clf()
                imod.idf.write(
                    f"data/5-visualization/{modelname}/zoutvracht_-0.5/{deelgebied}_{name}.idf", data
                )
